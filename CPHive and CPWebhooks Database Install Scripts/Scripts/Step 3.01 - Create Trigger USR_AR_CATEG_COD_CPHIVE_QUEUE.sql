--Created Via Ezno package. Package created on 7/31/2019. Trigger was last modified on: 7/30/2019 5:01:58 PM
IF EXISTS (SELECT name FROM sysobjects WHERE name = N'USR_AR_CATEG_COD_CPHIVE_QUEUE')
DROP TRIGGER [dbo].[USR_AR_CATEG_COD_CPHIVE_QUEUE]
GO
-- =============================================
...
-- Description:	
-- =============================================
CREATE TRIGGER [dbo].[USR_AR_CATEG_COD_CPHIVE_QUEUE] 
   ON  [dbo].[AR_CATEG_COD] 
...
BEGIN
	...
	SET @UPD_TABLE = 'customer_category'
	...
	IF(@UPD_TYP = 'U')
		...
		IF(NOT EXISTS (select TOP 1 1 from INFORMATION_SCHEMA.COLUMNS where substring(columns_updated(), (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) / 8 + 1, 1) & power(2, (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) % 8 ) > 0
			and table_name = 'AR_CATEG_COD' AND INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME NOT IN ('ROW_TS','RS_UTC_DT','RS_STAT')))
			RETURN
		END
	...
	IF(@FILTER IS NULL)
		...
		SELECT DISTINCT '{"CATEG_COD":"' + dbo.[fnUsrCleanJSON](CATEG_COD) + '"}' AS UPD_PK FROM INSERTED 
		UNION 
		SELECT DISTINCT '{"CATEG_COD":"' + dbo.[fnUsrCleanJSON](CATEG_COD) + '"}' AS UPD_PK FROM DELETED
		END
	ELSE
		BEGIN
		...
		CREATE TABLE [#TMP_AR_CATEG_COD] (
			[CATEG_COD] [varchar] (10) NULL
		)

		INSERT INTO [#TMP_AR_CATEG_COD]
		SELECT DISTINCT CATEG_COD FROM INSERTED
		UNION
		SELECT DISTINCT CATEG_COD FROM DELETED

		DECLARE @SQL VARCHAR(MAX)
		SET @SQL = 'INSERT INTO [#TMP_UPDATED]
		SELECT DISTINCT ''{"CATEG_COD":"'' + dbo.[fnUsrCleanJSON](T.CATEG_COD) + ''"}'' AS UPD_PK 
		FROM [#TMP_AR_CATEG_COD] T JOIN AR_CATEG_COD ON T.CATEG_COD = AR_CATEG_COD.CATEG_COD WHERE ' + @FILTER
		EXEC(@SQL)
		END
