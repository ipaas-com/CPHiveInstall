--Created Via Ezno package. Package created on 7/31/2019. Trigger was last modified on: 7/30/2019 5:02:32 PM
IF EXISTS (SELECT name FROM sysobjects WHERE name = N'USR_AR_CUST_CPHIVE_QUEUE')
DROP TRIGGER [dbo].[USR_AR_CUST_CPHIVE_QUEUE]
GO
...
-- Description:	
-- =============================================
CREATE TRIGGER [dbo].[USR_AR_CUST_CPHIVE_QUEUE] 
   ON  [dbo].[AR_CUST] 
...
BEGIN
	...
	IF(@UPD_TYP = 'U')
		...
		IF(NOT EXISTS (select TOP 1 1 from INFORMATION_SCHEMA.COLUMNS where substring(columns_updated(), (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) / 8 + 1, 1) & power(2, (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) % 8 ) > 0
			and table_name = 'AR_CUST' AND INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME NOT IN ('ROW_TS','RS_UTC_DT','RS_STAT')))
			RETURN
		END
	...
	IF(@FILTER IS NULL)
		...
		SELECT DISTINCT '{"CUST_NO":"' + dbo.[fnUsrCleanJSON](CUST_NO) + '"}' AS UPD_PK FROM INSERTED 
		UNION 
		SELECT DISTINCT '{"CUST_NO":"' + dbo.[fnUsrCleanJSON](CUST_NO) + '"}' AS UPD_PK FROM DELETED 
		END
	ELSE
		BEGIN
		...
		CREATE TABLE [#TMP_AR_CUST] (
			[CUST_NO] [varchar] (15) NULL
		)

		INSERT INTO [#TMP_AR_CUST]
		SELECT DISTINCT CUST_NO FROM INSERTED
		UNION
		SELECT DISTINCT CUST_NO FROM DELETED

		DECLARE @SQL VARCHAR(MAX)
		SET @SQL = 'INSERT INTO [#TMP_UPDATED]
		SELECT DISTINCT ''{"CUST_NO":"'' + dbo.[fnUsrCleanJSON](T.CUST_NO) + ''"}'' AS UPD_PK 
		FROM [#TMP_AR_CUST] T JOIN AR_CUST ON T.CUST_NO = AR_CUST.CUST_NO WHERE ' + @FILTER
		EXEC(@SQL)
		END
