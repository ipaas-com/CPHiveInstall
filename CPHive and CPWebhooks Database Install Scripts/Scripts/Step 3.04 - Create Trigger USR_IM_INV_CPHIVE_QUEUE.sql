--Created Via Ezno package. Package created on 7/31/2019. Trigger was last modified on: 7/31/2019 1:48:54 PM
IF EXISTS (SELECT name FROM sysobjects WHERE name = N'USR_IM_INV_CPHIVE_QUEUE')
DROP TRIGGER [dbo].[USR_IM_INV_CPHIVE_QUEUE]
GO
...
-- Description:	
-- =============================================
CREATE TRIGGER [dbo].[USR_IM_INV_CPHIVE_QUEUE] 
   ON  [dbo].[IM_INV] 
...
BEGIN
	...
	SET @UPD_TABLE = 'product_inventory'
	...
	IF(@UPD_TYP = 'U')
		...
		IF(NOT EXISTS (select TOP 1 1 from INFORMATION_SCHEMA.COLUMNS where substring(columns_updated(), (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) / 8 + 1, 1) & power(2, (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) % 8 ) > 0
			and table_name = 'IM_INV' AND INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME NOT IN ('ROW_TS','RS_UTC_DT','RS_STAT')))
			RETURN
		END
	...
	CREATE TABLE [#TMP_UPDATED_IM_INV] (
		[UPD_PK] [varchar] (800) NULL
	)

	IF(@FILTER IS NULL)
		BEGIN
		INSERT INTO [#TMP_UPDATED_IM_INV](UPD_PK)
		SELECT DISTINCT '{"ITEM_NO":"' + dbo.[fnUsrCleanJSON](ITEM_NO) + '", "LOC_ID":"' + dbo.[fnUsrCleanJSON](LOC_ID) + '"}' AS UPD_PK FROM INSERTED 
		UNION 
		SELECT DISTINCT '{"ITEM_NO":"' + dbo.[fnUsrCleanJSON](ITEM_NO) + '", "LOC_ID":"' + dbo.[fnUsrCleanJSON](LOC_ID) + '"}' AS UPD_PK FROM DELETED 
		END
	ELSE
		BEGIN
		...
		CREATE TABLE [#TMP_IM_INV] (
			[ITEM_NO] [varchar] (20) NULL,
			[LOC_ID] [varchar] (10) NULL
		)

		INSERT INTO [#TMP_IM_INV]
		SELECT DISTINCT ITEM_NO, LOC_ID FROM INSERTED
		UNION
		SELECT DISTINCT ITEM_NO, LOC_ID FROM DELETED

		DECLARE @SQL VARCHAR(MAX)
		SET @SQL = 'INSERT INTO [#TMP_UPDATED_IM_INV](UPD_PK)
		SELECT DISTINCT ''{"ITEM_NO":"'' + dbo.[fnUsrCleanJSON](T.ITEM_NO) + ''", "LOC_ID":"'' + dbo.[fnUsrCleanJSON](T.LOC_ID) + ''"}'' AS UPD_PK
		FROM [#TMP_IM_INV] T JOIN IM_INV ON T.ITEM_NO = IM_INV.ITEM_NO AND T.LOC_ID = IM_INV.LOC_ID WHERE ' + @FILTER
		EXEC(@SQL)
		END
	...
	SET UPD_DT = GETDATE(),
		...
		ATTEMPTS = 0,
		RETRY_DT = NULL
	FROM [#TMP_UPDATED_IM_INV] UPDATED
