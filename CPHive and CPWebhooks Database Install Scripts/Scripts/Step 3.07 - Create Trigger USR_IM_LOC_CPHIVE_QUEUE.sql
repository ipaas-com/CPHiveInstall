--Created Via Ezno package. Package created on 7/31/2019. Trigger was last modified on: 7/30/2019 5:04:20 PM
IF EXISTS (SELECT name FROM sysobjects WHERE name = N'USR_IM_LOC_CPHIVE_QUEUE')
DROP TRIGGER [dbo].[USR_IM_LOC_CPHIVE_QUEUE]
GO
-- =============================================
...
-- Description:	
-- =============================================
CREATE TRIGGER [dbo].[USR_IM_LOC_CPHIVE_QUEUE] 
   ON  [dbo].[IM_LOC] 
...
BEGIN
	...
	IF(@UPD_TYP = 'U')
		...
		IF(NOT EXISTS (select TOP 1 1 from INFORMATION_SCHEMA.COLUMNS where substring(columns_updated(), (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) / 8 + 1, 1) & power(2, (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) % 8 ) > 0
			and table_name = 'IM_LOC' AND INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME NOT IN ('ROW_TS','RS_UTC_DT','RS_STAT')))
			RETURN
		END
	...
	IF(@FILTER IS NULL)
		...
		SELECT DISTINCT '{"LOC_ID":"' + dbo.[fnUsrCleanJSON](LOC_ID) + '"}' AS UPD_PK FROM INSERTED 
		UNION 
		SELECT DISTINCT '{"LOC_ID":"' + dbo.[fnUsrCleanJSON](LOC_ID) + '"}' AS UPD_PK FROM DELETED
		END
	ELSE
		BEGIN
		...
		CREATE TABLE [#TMP_IM_LOC] (
			[LOC_ID] [varchar] (10) NULL
		)

		INSERT INTO [#TMP_IM_LOC]
		SELECT DISTINCT LOC_ID FROM INSERTED
		UNION
		SELECT DISTINCT LOC_ID FROM DELETED

		DECLARE @SQL VARCHAR(MAX)
		SET @SQL = 'INSERT INTO [#TMP_UPDATED]
		SELECT DISTINCT ''{"LOC_ID":"'' + dbo.[fnUsrCleanJSON](T.LOC_ID) + ''"}'' AS UPD_PK
		FROM [#TMP_IM_LOC] T JOIN IM_LOC ON T.LOC_ID = IM_LOC.LOC_ID WHERE ' + @FILTER
		EXEC(@SQL)
		END
