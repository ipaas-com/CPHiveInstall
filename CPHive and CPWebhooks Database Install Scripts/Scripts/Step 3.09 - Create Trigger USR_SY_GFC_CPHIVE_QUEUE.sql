--Created Via Ezno package. Package created on 7/31/2019. Trigger was last modified on: 7/30/2019 5:11:46 PM
IF EXISTS (SELECT name FROM sysobjects WHERE name = N'USR_SY_GFC_CPHIVE_QUEUE')
DROP TRIGGER [dbo].[USR_SY_GFC_CPHIVE_QUEUE]
GO

-- =============================================
-- Author:		Vance Berisford
-- Create date: 01/31/19
-- Description:	
-- =============================================
CREATE TRIGGER [dbo].[USR_SY_GFC_CPHIVE_QUEUE] 
   ON  [dbo].[SY_GFC] 
...
BEGIN
	...
	IF(@UPD_TYP = 'U')
		...
		IF(NOT EXISTS (select TOP 1 1 from INFORMATION_SCHEMA.COLUMNS where substring(columns_updated(), (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) / 8 + 1, 1) & power(2, (-1 + columnproperty(object_id(table_schema + '.' + table_name, 'U'), column_name, 'columnId')) % 8 ) > 0
			and table_name = 'SY_GFC' AND INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME NOT IN ('ROW_TS','RS_UTC_DT','RS_STAT')))
			RETURN
		END
	...
	CREATE TABLE [#TMP_UPDATED_SY_GFC] (
		[UPD_PK] [varchar] (800) NULL
	)

	IF(@FILTER IS NULL)
		BEGIN
		INSERT INTO [#TMP_UPDATED_SY_GFC]
		SELECT DISTINCT '{"GFC_NO":"' + dbo.[fnUsrCleanJSON](GFC_NO) + '"}' AS UPD_PK FROM INSERTED 
		UNION 
		SELECT DISTINCT '{"GFC_NO":"' + dbo.[fnUsrCleanJSON](GFC_NO) + '"}' AS UPD_PK FROM DELETED 
		END
	ELSE
		BEGIN
		...
		CREATE TABLE [#TMP_SY_GFC] (
			[GFC_NO] [varchar] (30) NULL
		)

		INSERT INTO [#TMP_SY_GFC]
		SELECT DISTINCT GFC_NO FROM INSERTED
		UNION
		SELECT DISTINCT GFC_NO FROM DELETED

		DECLARE @SQL VARCHAR(MAX)
		SET @SQL = 'INSERT INTO [#TMP_UPDATED_SY_GFC]
		SELECT DISTINCT ''{"GFC_NO":"'' + dbo.[fnUsrCleanJSON](T.GFC_NO) + ''"}'' AS UPD_PK
		FROM [#TMP_SY_GFC] T JOIN SY_GFC ON T.GFC_NO = SY_GFC.GFC_NO WHERE ' + @FILTER
		EXEC(@SQL)
		END
	...
	SET UPD_DT = GETDATE(),
		...
		ATTEMPTS = 0,
		RETRY_DT = NULL
	FROM [#TMP_UPDATED_SY_GFC] UPDATED
